...
class SIN(nn.Module):
    def __init__(self, latent_dim=cfg.latent_dim):
        ...
        self.lin = nn.Sequential(nn.Linear(latent_dim, 64), nn.ReLU(),
                                 nn.Linear(64, 128), nn.ReLU(), nn.Dropout1d(0.5),
                                 nn.Linear(128, 256), nn.ReLU(), nn.Dropout1d(0.5), 
                                #  nn.Linear(256, 512), nn.ReLU(), nn.Dropout1d(0.5), 
                                #  nn.Linear(512, 256), nn.ReLU(),nn.Dropout1d(0.5),
                                 nn.Linear(256,128), nn.ReLU(),nn.Dropout1d(0.5), 
                                 nn.Linear(128, 64), nn.ReLU(),nn.Dropout1d(0.5),
                                 nn.Linear(64, 1))
